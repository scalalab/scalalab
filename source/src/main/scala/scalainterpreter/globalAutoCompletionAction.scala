  ...
  def actionPerformed(e: ActionEvent) {
    ...
    if (selectedStr != null) {
      if (GlobalValues.autocompletionEnabled == false) {
        // enable autocompletion on the first Ctrl-Space press
      ...
      if (matches != null) {
        ...
        topLevelResultsList.addListSelectionListener(new ListSelectionListener() {
          override def valueChanged(lse: ListSelectionEvent) {
            ...
            selValue = selValue.substring(selValue.indexOf(GlobalValues.smallNameFullPackageSeparator) + 3, selValue
              .length())
            GlobalValues.selectedStringForAutoCompletion = selValue
          }
        })

        GlobalValues.autoCompletionFrame = new AutoCompletionFrame("scalalabConsole AutoCompletion ( F1 for detailed " +
          "help on the selected entry)")
