...
KeyStroke, OverlayLayout, ScrollPaneConstants, SwingWorker, JList}
...
class EditorPane extends JPanel with CustomizableFont {
  ...
  def myProcessingKeys(ke: KeyEvent): KeyEvent = {
    ...
    keycode match {
      ...
      case KeyEvent.VK_F9 => javax.swing.JOptionPane.showMessageDialog(null, "You can compile Java code by placing " +
        "the code within the main window!! ");
        ke
      ...
      case KeyEvent.VK_F1 =>
        ...
        if (selectedStr != null) {
          if (GlobalValues.autocompletionEnabled == false) {
            // enable autocompletion on the first F1 press
          ...
          if (matches != null) {
            ...
            topLevelResultsList.addListSelectionListener(new ListSelectionListener() {
              override def valueChanged(lse: ListSelectionEvent) {
                ...
                selValue = selValue.substring(selValue.indexOf(GlobalValues.smallNameFullPackageSeparator) + 3,
                  selValue.length())
                GlobalValues.selectedStringForAutoCompletion = selValue
              }
            })

            GlobalValues.autoCompletionFrame = new AutoCompletionFrame("scalalabConsole AutoCompletion ( F1 for " +
              "detailed help on the selected entry)")
      ...
      case KeyEvent.VK_F11 => // use Scala autocompletion to complete the user expression
        ...
        if (F11_consumed == false) {
             ...
             // check for multiple commands per line       
                    ...
                    if (previousSemicolonCommandStartIdx > lastNewLineCommandStartIdx)  {  // multiple commands per
                    line separated by ';''
                    ...
                    for ( k <- 0 until toksCnt)  {
                      var tokIndex = cstr.lastIndexOf(toksToCheckForLastIndex.charAt(k))+1  // index of token  (in
                      order to match the text after it)
                    if (tokIndex > lastNewLineCommandStartIdx)  {
                     ...
                     var  afterDot=""
                     var  membersRequested = false;  // when the user presses dot, we imply that the completion
                     should provide the members of a structure
               ...
               if (matchedSize > 0) {  // top level elements exist, edit them
                ...
                topLevelResultsList.addListSelectionListener(new ListSelectionListener() {
                     override def  valueChanged(lse: ListSelectionEvent ) {
                             ...
                             GlobalValues.selectedStringForAutoCompletion = selValue
                             editorPane.setCaretPosition(editorPane.getText().length())   	// set cursor at the end
                             of the text area
           ...
           GlobalValues.autoCompletionFrame = new AutoCompletionFrame("scalalabConsole AutoCompletion, Workspace
           variables");
           GlobalValues.autoCompletionFrame.displayMatches(topLevelResultsList);
                        ...
                        while (complResults.hasNext)  {   // collect the matched results
                            ...
                            if (afterDotCharsExist)  {   // user typed after "."
                             if (currentCompletion.startsWith(afterDot))  {
                                var  classMatchedMethods =  scalaExec.gui.ConsoleKeyHandler.getMethodsOfClass
                                (classOfItem, currentCompletion)
...
//  Press F2 to execute code up to cursor position     
 ...
 // Press  Shift-F4 on an identifier (either selected or not). Presents a completion list, if a dot exists the
 results are filtered, e.g.   with:  "jf.setV", filters only methods starting with "setV"
  // Press F4 on a type (e.j. javax.swing.JFrame). Presents the constructors, methods and fields of the type with a
  JTree)
...
var isalphaNumeric = (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <=
'9') || (ch == '_')
...
case scala.tools.nsc.interpreter.Results.Success => status ("Ok. <" + scalaExec.Interpreter.GlobalValues
.globalInterpreter.mostRecentVar + ">")
