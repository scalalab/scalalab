...
object ScalaInterpreterPaneGlobals {
  ...
  var completer: Completion.ScalaCompleter = null
  // the completer class of the Scala Interpreter on which some completion services are based
  var defaultImportsProcessed = false
  var defaultCursor = scalaExec.Interpreter.GlobalValues.editorPane.getCursor
  var lastResult = scala.tools.nsc.interpreter.Results.Success
  // last result evalutaed by the Scala interpreter
...
class ScalaInterpreterPane extends JPanel with CustomizableFont {
  ...
  def resetInterpreter(inter: scala.tools.nsc.interpreter.IMain) = {
    interpreterVar = Some(inter)
  }
  ...
  if (GlobalValues.startupHelpFlag)
    initialText = """ // Type ScalaLab code here (use `Configuration` menu to control the presentation of this
                  startup text).
//  Press  F6 (or Ctrl-E) to execute selected text  or current line (within the Event Dispatch Thread - Is more
reliable but Swing is frozen until command completes)
//  Press Shift-F6     to execute selected text  or current line in non-blocking mode (i.e. not in the Event Dispatch
Thr ead)
...
//   Press F7 uses code completion of the Scala Completer, can be used also for package completion, e.g. "javax.<F7>"
dis  plays the contents of package javax
...
//  Press Shift-F4 on a type (e.j. javax.swing.JFrame). Presents the constructors, methods and fields of the type
with a JTree)
...
//  Press TAB expands the abbreviation internal to the jsyntaxpane, e.g. bo<TAB> expands to Boolean (F8 displays
these abbreviations)
...
//  Press F12 to close all displayed figures
  ...
  GlobalValues.ggProgressInvis = new JComponent {
    // hide progress pane for JSyntaxPane
  ...
  def init {
    ...
    scalaExec.Interpreter.GlobalValues.editorPane.getDocument().putProperty(javax.swing.text.DefaultEditorKit
    ...
    (new SwingWorker[Unit, Unit] {
      override def doInBackground {
        ...
        scalaExec.Interpreter.GlobalValues.globalInterpreter = new scala.tools.nsc.interpreter.IMain(settings, new
            NewLinePrintWriter(out getOrElse (new ConsoleWriter), true)) {
        ...
        if (ScalaInterpreterPaneGlobals.defaultImportsProcessed == false) {
          ...
          _root_.scalaExec.Interpreter.GlobalValues.interpreterTypeForPane match {
            ...
            case _root_.scalaExec.Interpreter.GlobalValues.EJMLMat =>
              ...
              scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                .GlobalValues.basicImportsEJMLScala)
              if (GlobalValues.hostIsWin64 || GlobalValues.hostIsLinux64)
                scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                  .GlobalValues.warmUpGSLScript)
            ...
            case _root_.scalaExec.Interpreter.GlobalValues.CUDAMat =>
              ...
              scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                .GlobalValues.basicImportsCUDAScala)
              if (GlobalValues.hostIsWin64 || GlobalValues.hostIsLinux64)
                scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                  .GlobalValues.warmUpGSLScript)
              if (scalaExec.Interpreter.GlobalValues.userConsole.getText.contains("error: ")) {
            ...
            case _root_.scalaExec.Interpreter.GlobalValues.MTJMat =>
              ...
              scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                .GlobalValues.basicImportsMTJScala)
              if (GlobalValues.hostIsWin64 || GlobalValues.hostIsLinux64)
                scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                  .GlobalValues.warmUpGSLScript)
              if (scalaExec.Interpreter.GlobalValues.userConsole.getText.contains("error: ")) {
            ...
            case _root_.scalaExec.Interpreter.GlobalValues.ApacheCommonMathsMat =>
              ...
              scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                .GlobalValues.basicImportsCommonMathsScala)
              if (GlobalValues.hostIsWin64 || GlobalValues.hostIsLinux64)
                scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                  .GlobalValues.warmUpGSLScript)
              if (scalaExec.Interpreter.GlobalValues.userConsole.getText.contains("error: ")) {
            ...
            case _root_.scalaExec.Interpreter.GlobalValues.JBLASMat =>
              ...
              scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                .GlobalValues.basicImportsJBLASScala)
              if (GlobalValues.hostIsWin64 || GlobalValues.hostIsLinux64)
                scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                  .GlobalValues.warmUpGSLScript)
              if (scalaExec.Interpreter.GlobalValues.userConsole.getText.contains("error: ")) {
            ...
            case _root_.scalaExec.Interpreter.GlobalValues.EIGENMat =>
              ...
              scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                .GlobalValues.basicImportsEigenScala)
              if (GlobalValues.hostIsWin64 || GlobalValues.hostIsLinux64)
                scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(_root_.scalaExec.Interpreter
                  .GlobalValues.warmUpGSLScript)
              if (scalaExec.Interpreter.GlobalValues.userConsole.getText.contains("error: ")) {
    ...
    val ggScroll = new JScrollPane(scalaExec.Interpreter.GlobalValues.editorPane, VERTICAL_SCROLLBAR_ALWAYS,
  ...
  def interpret(code: String) {
    interpreterVar.foreach(globalInterpreter => {
      status(null)
      try {
        globalInterpreter.interpret(code) match {
          /* case IR.Error       => status( "! Error !" )
           case IR.Success     => status( "Ok. <" + scalaExec.Interpreter.GlobalValues.globalInterpreter
           .mostRecentVar + ">" )
          ...
          javax.swing.SwingUtilities.invokeLater(new Runnable {
            def run {
              ScalaInterpreterPaneGlobals.commandFuture = Future {
                ...
                var mem = GlobalValues.availMemFormat.format(GlobalValues.rt.freeMemory().asInstanceOf[Double] /
                  1000000)
