...
package object plot {
  ...
  def plot[X, Y, V](
    ...
    def getChartStuff(defaultName: (Int) => String, defaultColor: (Int) => Paint, defaultStroke: (Int) => Stroke):
  ...
  /*
  def image[M,V]
  (img : scalaSci.RichDouble2DArray,
   ...
   tips : PartialFunction[(Int,Int), String] = null.asInstanceOf[PartialFunction[(Int,Int), String]]):Series = new
   Series {

    val mt = img

    val (minx,maxx) = (0,img.Nrows)
    val (miny,maxy) = (0,img.Ncols)

    val items = img.getv.toIndexedSeq
