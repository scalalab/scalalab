...
class RichDouble2DArray(final var v: Array[Array[Double]]) extends AnyRef with
scalaSci.scalaSciMatrix[scalaSci.RichDouble2DArray] {

  import scalaSci.RichDouble2DArray._

  final val rdObj = this

  final var Nrows = v.length
  // number of rows
  final var Ncols = v(0).length
  ...
  final def getNativeMatrixRef() = v // the scalaSci.RichDouble2DArray does not wrap a Matrix class of a specific
  // library, thus return simply the data representation
...
//    println("thread = "+ threadId +", firstRow = "+firstRow + " lastRow = "+lastRow+" numRowsForThread =
// "+numRowsForThread)
