...
object SearchLibs {
  ...
  def GUISearchLibs() = {
    ...
    querySpec.setToolTipText("type here keywords separated by &, e.g. Eig & ejml and then press the Search button to " +
      "serach")
  ...
  def searchAllLibs(jarFileNamesOfLibs: Array[String], keywords: Array[String]) = {
    ...
    for (jarFileName <- jarFileNamesOfLibs) {
            ...
            if (javaName.indexOf("$") == -1) {
              ...
              try {
                ...
                if (foundClass != null) {
                  ...
                  for (currentMethod <- classMethods) {
                    ...
                    if (Modifier.isPublic(currentMethod.getModifiers())) {
                      ...
                      if (containsAllKeywords(methodName, keywords, matchCase)) {
                        // relevant method
                        ...
                        for (param <- paramTypes) {
                          // construct a parameter list description
                          if (firstParam)
