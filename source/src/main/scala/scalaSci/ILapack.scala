...
*/
  ...
  // returns: (rWr, rWi, lEigenVecs) i.e. real parts of the right eigenvalues, imag parts of the right eigenvalues,
  // left eigenvectors
   ...
   */
  /*
   Based on dgesdd that computes thye singular value decomposition (SVD) of a m-by-n matrix  A, optionally computing
   the left
   ...
   The SVD is written
    ...
    The diagonal elements of Ξ£, are the singular values of A; they are real and non-negative, and are returned in
    descending order.
  ...
  */
  ...
  def DGELS(A: Array[Array[Double]], B: Array[Array[Double]]) = {
    ...
    while (c < NRHS) {
      // for all RHS
      var v = 0
      while (v < solVecLength) {
        // for all variables of the system
  ...
  def DGELD(A: Array[Array[Double]], B: Array[Array[Double]]) = {
    ...
    var rcond = -1.0 // used to determine the effective rank of  A. Singular values S(i) <= rcond*S(1) are treated as
    // zero. If rcond<0 machine precision is used instead
    var M = A.length // number of rows of matrix A
    var N = A(0).length // number of columns of matrix A
    var NRHS = B(0).length; // number of right hand sides, i.e., the number of columns of the matrices B and X
    ...
    LAPACK.getInstance().dgelsd(M, N, NRHS, dAc.getData(), LDA, dBc.getData(), LDB, S, rcond, rank, work, workSize,
      iwork, info)
    workSize = work(0).asInstanceOf[Int] // take the computed optimal workspace size
