...
trait scalaSciMatrix[specificMatrix] {
  var Nrows: Int
  // keeps  the number of rows of the Matrix
  var Ncols: Int //  keeps  the number of columns of the Matrix

  val LargeMatrix = 100
  final val thisObj = this

  // keeps a reference to the object where the scalaSciMatrix trait is fixed in
  def numRows(): Int
  ...
  */
  ...
  //  return the matrix from the native representation whatever the native representation is. In that way we can
  ...
  //  gets the element at row r and column c. 
  def update(r: Int, c: Int, value: Double): Unit // sets the element at row r and column c to the corresponding value 

  //  the generic apply() methods follow, that are implemented in terms of the two abstract apply and update methods
  // (defined above)
  //  the functionality of the abstract methods is defined by the patricular matrix type that mixes-in the
  // scalaSciMatrix[specificMatrix] trait
  ...
  // extracts a submatrix specifying rows only, take all columns, e.g. m(2, 4, 12, ::) corresponds to Matlab's m
  // (2:4:12, :)'
  def apply(rowL: Int, rowInc: Int, rowH: Int, allColsSymbol: scala.::.type): specificMatrix = {
    ...
    if (diff>0.0)
      ...
      */
      ...
      var colStart = 0;
      var colEnd = Ncols - 1 // all columns
  ...
  def apply(rowsSpec: scalaSci.MatlabRange.MatlabRangeNext, colsSpec: scalaSci.MatlabRange.MatlabRangeNext):
  specificMatrix = {
    var rowL = rowsSpec.mStart.inc.toInt
    var rowH = rowsSpec.mStart.endv.toInt
    var rowInc = 1
  ...
  def apply(rowLow: Int, rowInc: Int, rowHigh: Int, colLow: Int, colInc: Int, colHigh: Int): specificMatrix = {
    ...
    if (rowStart <= rowEnd && colStart <= colEnd) {
      // positive increment at rows and columns
      var crow = rowStart // indexes current row
      var ccol = colStart // indexes current column
      var rowIdx = 0;
      var colIdx = 0 // indexes at the new Matrix
...
def update (rowSpec: scalaSci.MatlabRange.MatlabRangeNext, colSpec: scalaSci.MatlabRange.MatlabRangeNext, v: Double)
: Unit = {
var rowL = rowSpec.mStart.inc.toInt
var rowH = rowSpec.mStart.endv.toInt
...
var cinc = cincp
var rlow = rlowp;
var rhigh = rlow + mrM * rinc
var clow = clowp;
var chigh = clow + mrN * cinc
...
var lcolidx = clow
while (rrow < mr.Nrows) {
// for all rows of the right-hand side matrix
rcol = 1
lcolidx = clow // starting column within the "subassigned" matrix
while (rcol < mr.Ncols) {
// for all cols of the right-hand side matrix
...
if (rhigh >= Nrows || chigh >= Ncols) {
// dynamically increase the size of the matrix when subassigning out of its range
println ("accessing out of range element")
} // dynamically increase the size of the matrix
...
// receiver's dimensions
var sN = that.Nrows;
var sM = that.Ncols; // parameter's dimensions
...
Nrows = 1;
Ncols = aRows * aCols;
} // default to reshaping in a large row
