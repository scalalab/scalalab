...
vc(0)=200   // change at the cloned vector
  ...
  override final def set(k: Int, value: Double) = {
    data(k) = value
  }
...
// append element as last vector's element,  e.g. var data = vrand(2); var vv = 4.55 ::  data   // 4.55 is placed
...
// prepend element as first vector's element, e.g.  var data = vrand(2); var vv = 4.55 :::  data   // 4.55 is
// placed at the front of the vector
final def ::: (elem: Double): Vec = {
var v2 = new Array[Double] (length + 1)
v2 (0) = elem
...
// append element as last vector's element,  e.g.  var data = vrand(2); var vv =  data ::<  4.55 // 4.55 is placed
// as the last element of the vector
final def ::< (elem: Double): Vec = {
var v2 = new Array[Double] (length + 1)
Array.copy (data, 0, v2, 0, length)
...
// prepend element as first vector's element,  e.g.  var data = vrand(2); var vv =  data :::< 4.55   // 4.55 is
// placed at the front of the vector
final def :::< (elem: Double): Vec = {
var v2 = new Array[Double] (length + 1)
v2 (0) = elem
...
if (k >= length) {
// increase the size of the vector dynamically
...
if (vl_rhs != rangeLen) {
// improper sizes of the involved vector subranges for assignment
println ("target vector subrange and right-hand size vector lengths do not agree, i.e. v1_rhs =  " + vl_rhs + "," +
" rangeLen = " + rangeLen)
this // return the vector unaltered
}

if (high >= length) {
// dynamically increase the size of the Vector
...
//   if the receiver's length is 1 expand the receiver filling with the value of the 0th element (since implicit
...
object Vec {
  ...
  var vecResizeFactor = 1.5

  // controls by how much to increase dynamically the size of the vector }
  final def setVecResizeFactor(newResizeFactor: Double): Double = {
    var prevResizeFactor = vecResizeFactor;
    vecResizeFactor = newResizeFactor;
    prevResizeFactor
  }
