...
class ComplexMatrix(n: Int, m: Int) {
  ...
  var Ncols = m
  //  keeps  the number of columns of the Matrix
  ...
  def setv(values: Array[Array[Double]], n: Int, m: Int) = {
    v = values;
    Nrows = n;
    Ncols = m
  }
  ...
  def set(row: Int, column: Int, re: Double, im: Double) = {
    v(row)(2 * column) = re;
    v(row)(2 * column + 1) = im
  }
...
def this (Nrows: Int, Ncols: Int, df: Double) = {
...
object ComplexMatrix {
  ...
  var digitsPrecision = 4
  // controls pecision in toString()
  var mxRowsToDisplay = 6
  var mxColsToDisplay = 6

  def setDigitsPrecision(n: Int) = {
    digitsPrecision = n
  }

  def setRowsToDisplay(nrows: Int) = {
    mxRowsToDisplay = nrows
  }

  def setColsToDisplay(ncols: Int) = {
    mxColsToDisplay = ncols
  }
