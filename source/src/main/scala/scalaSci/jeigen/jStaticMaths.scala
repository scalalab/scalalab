...
//  This one is for the 0-indexed  JEIGEN based Matrix 


object StaticMathsEigen extends AnyRef with scalaSci.StaticScalaSciCommonOps[scalaSci.jeigen.Mat] with
scalaSci.StaticScalaSciGlobal {

  implicit final def vecToMat(x: Vec): scalaSci.jeigen.Mat = {
    vec2Mat(x)
  }

  // implicit conversion of a Vec to Mat
  implicit final def MatToDoubleArr(x: scalaSci.jeigen.Mat) = {
    x.getRow(0)
  }
  ...
  def computeProperMatrixSizeThresholdForMultithreading = {
    ...
    var breakFlag = false
    var m = rand(testedSize.asInstanceOf[Int], testedSize.asInstanceOf[Int]) // create a matrix to test the serial
    // case vs the multithreaded case
    var iterCnt = 0
