...
class Matrix(n: Int, m: Int) extends AnyRef with scalaSciMatrix[scalaSci.Matrix] {
  ...
  // the scalaSci.Matrix does not wrap a Matrix class of a specific library, thus return simply the data
  // representation
  final def getNativeMatrixRef() = v

  final def matFromNative() = new Matrix(v)

  final def matFromNative(v: Array[Array[Double]]) = new Matrix(v)

  final def setv(values: Array[Array[Double]], n: Int, m: Int) = {
    v = values;
    Nrows = n + 1;
    Ncols = m + 1
  }

  override final def size() = (Nrows - 1, Ncols - 1)
  ...
  final def this(Nrows: Int, Ncols: Int, df: Double) = {
    // initializes to a default value
...
var ccol = colStart
var rowIdx = 1;
var colIdx = 1 // indexes at the new Matrix
while (crow <= rowEnd) {
ccol = colStart;
colIdx = 1
while (ccol <= colEnd) {
