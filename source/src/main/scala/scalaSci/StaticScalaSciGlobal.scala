...
trait StaticScalaSciGlobal {
  ...
  implicit final def vecToMatrix(x: Vec): Matrix = {
    vec2Matrix(x)
  } // implicit conversion of a Vec to Matrix

  implicit final def DoubleToRichNumber(x: Double) = {
    new RichNumber(x)
  }

  // implicit conversion of a Double number to RichNumber
  implicit final def IntToRichNumber(x: Int) = {
    new RichNumber(x)
  }

  // implicit conversion of a Int number to RichNumber
  implicit final def ShortToRichNumber(x: Short) = {
    new RichNumber(x)
  }

  // implicit conversion of a Short number to RichNumber
  implicit final def LongToRichNumber(x: Long) = {
    new RichNumber(x)
  } // implicit conversion of a Long number to RichNumber

  implicit final def DoubleArrayToRichDouble1DArray(x: Array[Double]) = {
    new RichDouble1DArray(x)
  }

  // implicit conversion of an Array[Double] to RichDoubleNumber
  implicit final def DoubleDoubleArrayToRichDouble1DArray(x: Array[Array[Double]]) = {
    new RichDouble2DArray(x)
  } // implicit conversion of an Array[Array[Double]] to RichDouble2DArray

  implicit final def RichDouble1DArrayToVec(x: RichDouble1DArray) = {
    new Vec(x.getv)
  }

  // implicit conversion of a RichDouble1DArray to Vec
  implicit final def RichDouble1DArrayToDoubleArray(x: RichDouble1DArray) = {
    x.getv
  }

  // implicit conversion of a RichDouble1AArray to double []
  implicit final def RichDouble2DArrayToDoubleDoubleArray(x: RichDouble2DArray) = {
    x.getv
  } // implicit conversion of a RichDouble2DArray to double [][]


  // used for plotting routines
  implicit final def VecToDoubleArr(x: Vec) = {
    x.getv
  }

  implicit final def MatrixToDoubleArr(x: Matrix) = {
    x.getRow(1)
  }
  ...
  final def AAD(s: String) = {
    ...
    var i = 0
    while (i < s.length - 1) {
      // count how many rows are specified
      if (s(i) == ';')
        nRows += 1
      i += 1
    }
  ...
  // Signal Processing Routines
  final def fft(matr: Array[Array[Double]]): (Array[Array[Double]], Array[Array[Double]]) = scalaSci.FFT.FFTScala.fft

  (matr)

  final def fft(sig: Vec): (Array[Double], Array[Double]) = scalaSci.FFT.FFTScala.fft(sig.getv)

  final def fft(sig: Array[Double], SF: Double): (Array[Double], Array[Double], Array[Double]) = scalaSci.FFT
    .FFTScala.fft(sig, SF)

  //scalaSci.FFT.ApacheFFT.fft(sig)


  final def fft(sig: Vec, SF: Double): (Array[Double], Array[Double], Array[Double]) = scalaSci.FFT.FFTScala.fft(sig
    .getv, SF)
  ...
  final def ifft(realffts: Array[Double], imffts: Array[Double]): Array[Double] = scalaSci.FFT.FFTScala.ifft

  (realffts, imffts)

  final def IFFT(sig: Array[org.apache.commons.math3.complex.Complex]): Array[org.apache.commons.math3.complex
  .Complex] = {
  ...
  final def length(a: scalaSci.Matrix): Int = {
    a.length
  }
...
*/
  ...
  */
  ...
  final def real(cm: ComplexMatrix) = cm.real

  // get the real parts as a RichDouble2DArray
...
covariance(x, x)
  ...
  final def covariance(v1: Array[Double], v2: Array[Double]) = {
    // TODO: check
    var n = v1.length
    var degrees = n - 1
    var c = 0.0;
...
Nrows = 1;
Ncols = aRows * aCols;
} // final default to reshaping in a large row
...
// keep two set of indices, i.e. iorig, jorig: indices at the original matrix and inew, jnew: indices at the new
// matrix
var iorig = 1;
var jorig = 1;
var inew = 1;
...
jnew += 1
if (jnew >= Ncols) {
// next row of the reshaped new matrix
jnew = 1
inew += 1
}
} // jorig < aCols
...
var Nrows = matr.Nrows;
var Ncols = matr.Ncols
var sm = 0.0
...
final def variance (v: Matrix): RichDouble1DArray = {
// TODO: check
var n = v.Nrows;
var m = v.Ncols;
...
final def stddeviation (v: Matrix): RichDouble1DArray = {
// TODO: check
...
final def covariance (v1: Matrix, v2: Matrix): Matrix = {
// TODO: check
var n = v1.Nrows
var m1 = v1.Ncols
var m2 = v2.Ncols
...
final def covariance (v: Matrix): Matrix = {
// TODO: check
var n = v.Nrows
var m = v.Ncols
...
var Nrows = ar.Nrows
var dvalr = new Array[Double] (Nrows);
var dvali = new Array[Double] (Nrows);
var dvr = Array.ofDim[Double] (Nrows, Nrows);
var dvi = Array.ofDim[Double] (Nrows, Nrows);
...
em (6) = 10.0;
var dvalr = new Array[Double] (Nrows);
var dvali = new Array[Double] (Nrows + 1);
var dvr = Array.ofDim[Double] (Nrows, Nrows);
var dvi = Array.ofDim[Double] (Nrows, Nrows);
...
em (8) = 5;
// the maximum allowed number of inverse iterations for the calculation of each calculation
...
// the calculated eigenvectors are delivered in the columns of vec; an eigenvector corresponding to a real
// eigenvalue given in array re
// is delivered in the corresponding column of array vec;
// the real and imaginary part of an eigenvector corresponding to the first member of a nonreal complex conjugate
// pair
// of eigenvalues given in the arrays re, im are delivered in the two consecutive columns of array vec
// corresponding to this pair
// (the eigenvectors corresponding to the second members of nonreal complex complex conjugate pairs are not
// delivered,
...
val numR = dm.getNumRows ();
val numC = dm.getNumCols ()
...
//final def eig(): (scalaSci.RichDouble2DArray, scalaSci.RichDouble1DArray) = { (new scalaSci.RichDouble2DArray(1,
// 1),  new scalaSci.RichDouble1DArray(1)) }  // ???
