...
//   This trait is analogous to the scalaSciMatrix trait by for static operations.
...
trait StaticScalaSciCommonOps[specificMatrix] {
  ...
  final def size(A: scalaSciMatrix[specificMatrix]): (Int, Int) = A.size
  ...
  final def fromDoubleArray(x: Array[Array[Double]]) = {
    ...
    var newMat = scalaSci.MatrixFactory(this, nrows, ncols).asInstanceOf[scalaSciMatrix[specificMatrix]] // create a
    // Matrix
    var r = 0;
    var c = 0
    while (r < nrows) {
  ...
  final def ones0(numRows: Int, numCols: Int) = {
    var ret = scalaSci.MatrixFactory(this, numRows, numCols).asInstanceOf[scalaSciMatrix[specificMatrix]] // create a
    // Matrix
    var r, c = 0
    while (r < numRows) {
      c = 0
  ...
  final def M0(s: String) = {
    ...
    var numbersMatrix = scalaSci.MatrixFactory(this, nRows, nCols).asInstanceOf[scalaSciMatrix[specificMatrix]] //
    // create a Matrix
    var k = 0
    while (k < nRows) {
      // read array
  ...
  final def toDoubleArray(A: scalaSciMatrix[specificMatrix]) = A.toDoubleArray // returns a Java/Scala 2-D array
  // representation of the matrix contents
