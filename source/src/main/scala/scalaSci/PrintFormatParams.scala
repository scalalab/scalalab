...
object PrintFormatParams {
  ...
  // controls verbosing the results of  toString()
  def setVerbose(vflag: Boolean) = {
    verboseFlag = vflag
  }

  def getVerbose() = verboseFlag

  var vecDigitsPrecision = 4

  // controls pecision in toString()
  def getVecDigitsPrecision() = vecDigitsPrecision

  var vecMxElemsToDisplay = 20

  // controls maximum number of Vector elements to display
  def setVecMxElemsToDisplay(mxElems: Int): Int = {
    var prevElems = vecMxElemsToDisplay;
    vecMxElemsToDisplay = mxElems;
    prevElems
  }

  def setVecDigitsPrecision(precision: Int) = {
    vecDigitsPrecision = precision
  }

  var matDigitsPrecision = 4
  ...
  def setMatMxRowsToDisplay(nrows: Int) = {
    matMxRowsToDisplay = nrows
  }

  def setMatMxColsToDisplay(ncols: Int) = {
    matMxColsToDisplay = ncols
  }

  def setMatDigitsPrecision(precision: Int) = {
    matDigitsPrecision = precision;
    vecDigitsPrecision = precision
  }
