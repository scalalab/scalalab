...
// this class provides Matrix operations, by wrapping the Efficient Java Matrix Library and using block matrix
// operations
class BMat(bmi: BlockMatrix64F) extends AnyRef {
  var bm = bmi
  // keep the reference to the Block Matrix
  // getters for size
  ...
  final def apply(row: Int, col: Int, resize: Boolean) = {
  ...
  final def apply(rowsSpec: scalaSci.MatlabRange.MatlabRangeNext, colsSpec: scalaSci.MatlabRange.MatlabRangeNext):
  BMat = {
    var rowL = rowsSpec.mStart.inc.toInt
    var rowH = rowsSpec.mStart.endv.toInt
    var rowInc = 1
