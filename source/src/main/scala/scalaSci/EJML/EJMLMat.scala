...
class Mat(smi: SimpleMatrix) extends AnyRef with scalaSci.scalaSciMatrix[scalaSci.EJML.Mat] {
  ...
  final def printEJML(numChar: Int, precision: Int) = {
    sm.print(numChar, precision)
  }

  // Prints the matrix to standard out given a printf() style floating point format, e.g. print("%f").
  final def printEJML(format: String) = {
    sm.print(format)
  }

  // short print method
  final def pEJML(format: String) = {
    sm.print(format)
  }
  ...
  final def cc(that: Mat): Mat = {
    ...
    var Arows = this.Nrows;
    var Acolumns = this.Ncols
