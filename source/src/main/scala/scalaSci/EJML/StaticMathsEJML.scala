...
object StaticMathsEJML extends AnyRef with scalaSci.StaticScalaSciCommonOps[scalaSci.EJML.Mat] with
scalaSci.StaticScalaSciGlobal {
  ...
  final def setMemoryUsageFaster = {
    org.ejml.EjmlParameters.MEMORY = org.ejml.EjmlParameters.MemoryUsage.FASTER
  }

  final def setMemoryUsageLowMemory = {
    org.ejml.EjmlParameters.MEMORY = org.ejml.EjmlParameters.MemoryUsage.LOW_MEMORY
  }
  ...
  final def setMultColumnSwitch(multColumnSwitch: Int) = {
    org.ejml.EjmlParameters.MULT_COLUMN_SWITCH = multColumnSwitch
  }

  final def setMultTranabColumnSwitch(multTranabColumnSwitch: Int) = {
    org.ejml.EjmlParameters.MULT_TRANAB_COLUMN_SWITCH = multTranabColumnSwitch
  }

  // At which point should it switch to the block cholesky and qr algorithm.
  final def setSwitchNlock64Cholesky(switchBlock64Cholesky: Int) = {
    org.ejml.EjmlParameters.SWITCH_BLOCK64_CHOLESKY = switchBlock64Cholesky
  }

  final def setSwitchNlock64QR(switchBlock64QR: Int) = {
    org.ejml.EjmlParameters.SWITCH_BLOCK64_QR = switchBlock64QR
  }
  ...
  final def extractDiag(x: Mat) = x.extractDiag
...
*/
  ...
  final def createMatrixD(eig: EigenDecomposition[DenseMatrix64F]): scalaSci.EJML.Mat = {
    new Mat(new SimpleMatrix(EigenOps.createMatrixD(eig)))
  }
  ...
  final def createMatrixV(eig: EigenDecomposition[DenseMatrix64F]): scalaSci.EJML.Mat = {
