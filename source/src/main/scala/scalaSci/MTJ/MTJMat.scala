...
class Mat(Nin: Int, Min: Int) extends AnyRef with scalaSci.scalaSciMatrix[scalaSci.MTJ.Mat] {
  ...
  final def getNativeMatrixRef() = dm // the scalaSci.MTJ.Mat class wraps the no.uib.cipr.matrix.DenseMatrix class,
  ...
  final def *(that: Mat): Mat = {
    var mulMat = dm.multAdd(1.0, that.dm, new DenseMatrix(this.dm.numRows, that.dm.numColumns))
  ...
  final def eig() = {
    ...
    (new scalaSci.RichDouble2DArray(new Mat(evdObj.getRightEigenvectors()).toDoubleArray()), new scalaSci
    .RichDouble1DArray(evdObj.getRealEigenvalues()))
...
object Mat {
  ...
  final def setDisplayLength(n: Int) = {
    mxElemsToDisplay = n
  }
