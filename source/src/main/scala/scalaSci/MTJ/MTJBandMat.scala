...
class MTJBandMat(val N: Int, val kl: Int, val ku: Int) extends AnyRef {
  //  with scalaSci.scalaSciMatrix {
  ...
  final def size = (kl, ku)

  // returns the lower-band width and upper-band width
  ...
  // TODOTest
  final def getNativeMatrixRef() = bdm

  final def matFromNative(nm: BandMatrix) = {
    new MTJBandMat(nm.numRows(), nm.numSubDiagonals(), nm.numSuperDiagonals())
  }
  ...
  final def apply(n: Int, m: Int, resize: Boolean): Double = {
    // TODO-Sterg: implement resizing
    bdm.get(n, m)
  ...
  final def apply(rowL: Int, rowH: Int, allColsChar: ::.type): MTJBandMat = {
    ...
    if (rowStart <= rowEnd) {
      ...
      var ccol = colStart
      var rowIdx = 0;
      var colIdx = 0 // indexes at the new MTJBandMat
      while (crow >= rowEnd) {
        ccol = colStart;
        colIdx = 0
        while (ccol <= colEnd) {
