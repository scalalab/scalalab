...
class PlotController() {
  ...
  // set font
  def setFont(font: java.awt.Font) = {
    scalaSci.math.plot.PlotGlobals.defaultAbstractDrawerFont = font
  }
  ...
  // set line width
  def setlineWidth(lw: Int) = {
    scalaSci.math.plot.render.AbstractDrawer.line_width = lw
  }
  ...
  def setX(x: Array[Double]) = {
    xData = x
  }

  // sets the x-data for plotting
  def setY(y: Array[Double]) = {
    yData = y
  }

  // sets the y-data for plotting
  def setZ(z: Array[Double]) = {
    zData = z
  } // sets the z-data for plotting

  def setX(x: scalaSci.Vec) = {
    xData = x.getv
  }

  // sets the x-data using Vec
  def setY(y: scalaSci.Vec) = {
    yData = y.getv
  }

  // sets the y-data using Vec
  def setZ(z: scalaSci.Vec) = {
    zData = z.getv
  } // sets the z-data using Vec

  def setHoldOn() = {
    scalaSci.math.plot.plot.hold(true)
  }

  // set hold on @new
  def setHoldOff() = {
    scalaSci.math.plot.plot.hold(false)
  } // set hold off @new
  ...
  def setplotTitle2D(plTitle: String) = {
    plotTitle2D = plTitle
  }

  var plotTitle3D = "3-D Plot"

  def setplotTitle3D(plTitle: String) = {
    plotTitle3D = plTitle
  }
  ...
  def mkplot() = {
    scalaSci.math.plot.plotTypes.plot(xData, yData, scalaSci.math.plot.PlotGlobals.defaultAbstractDrawerColor,
      plotTitle2D)
  ...
  def mkplot3D() = {
    ...
    scalaSci.math.plot.plotTypes.plot(xData, yData, zData, scalaSci.math.plot.PlotGlobals.defaultAbstractDrawerColor,
      plotTitle3D)
    scalaSci.math.plot.plot.xlabel(xlabelStr)
    scalaSci.math.plot.plot.ylabel(ylabelStr)
