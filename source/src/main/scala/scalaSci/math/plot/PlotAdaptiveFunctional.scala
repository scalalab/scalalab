...
object PlotAdaptiveFunctional {
  var dxForDeriv = 0.000001
  // a very small increment to compute the function's derivative
  var thresholdSmallRateOfChange = 1.0
  // for smaller than this we use the default dx
  var thresholdLargeRateOfChange = 1000.0 // for very fast changing functions avoid taking too much samples

  def setDxForDeriv(newDx: Double) = {
    dxForDeriv = newDx
  }

  def setThresholdSmallRateOfChange(newThresholdSmallRateOfChange: Double) = {
    thresholdSmallRateOfChange = newThresholdSmallRateOfChange
  }

  def setThresholdLargeRateOfChange(newThresholdLargeRateOfChange: Double) = {
    thresholdLargeRateOfChange = newThresholdLargeRateOfChange
  }

  // adaptive function plotting. Plot a function by changing automatically the sampling rate in regions where the
  // function
  // changes quickly
  def faplot(f: Double => Double, low: Double, high: Double, nP: Int = 200) = {
    ...
    while (currx < high) {
      // currx steps across the whole sampling axis
