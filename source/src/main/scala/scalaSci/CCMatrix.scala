...
class CCMatrix {
  ...
  var ccm: CompColMatrix = _
  // the representation of the sparse matrix
  var numRows: Int = _
  var numColumns: Int = _
  var z = Array.ofDim[Int](1, 1)

  final def length() = numRows * numColumns
  ...
  def +(that: Double) = {
    ...
    while (miter.hasNext()) {
      var current = miter.next()
      var crow = current.row();
      var ccol = current.column()
