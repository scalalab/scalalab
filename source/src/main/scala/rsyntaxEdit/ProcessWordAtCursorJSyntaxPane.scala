...
object ProcessWordAtCursorJSyntaxPane {
  ...
  def processWordAtCursorJSyntaxPane(wd: String) = {
    ...
    if (typeOfId != "<notype>") {
      ...
      typeOfId = typeOfId filter (_ != '(') filter (_ != ')') // remove some strange parenthesis the interpreter
      // returns before type
      ...
      if (typeOfId.contains(":") == false) {
        ...
        //    sI.quietRun(execString)  // execute quitely, the required value is assigned to the synthetic variable
        // $$dummy

        //        var valueOfId = scalaExec.Interpreter.GlobalValues.globalInterpreter.valueOfTerm("$$dummy")
        // .getOrElse("none")
        ...
        if (GlobalValues.getValuesForAllRSyntax == true) {
          ...
          if (valueOfId != "none") {
            if (isScalaSciType == false && isPrimitiveType == false) {
              // not a scalaSci or primitive type, avoid displaying value
              editor.setToolTipText(wordAtCursor + " [ " + typeOfId + " ]  ")
            }
            else {
              //   var valueOfId = scalaExec.Interpreter.GlobalValues.globalInterpreter.valueOfTerm("$$dummy")
