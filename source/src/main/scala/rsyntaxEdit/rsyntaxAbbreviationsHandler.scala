...
object rsyntaxAbbreviationsHandler {
  ...
  def initAbbreviationMap() = {
    ...
    if (abbreviationsMapInited == false) {
      abbreviationsMap = scalalabEdit.EditUtils.buildAbbreviationsMap(scalaExec.Interpreter.GlobalValues.workingDir +
        java.io.File.separatorChar + "Abbreviations.txt")
