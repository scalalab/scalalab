...
// this action inspects a completion list for the type of an identifier, e.g. var jf = new javax.swing.JFrame, jf is
// the identifier
...
class RSyntaxInspectCompletionListAction() extends javax.swing.AbstractAction() {
  ...
  override def actionPerformed(e: ActionEvent) {
    ...
    while (offset >= 0 && exited == false) {
      ...
      var isalphaNumeric = (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || ch ==
        '.' || ch == '_' || ch == '$'
    ...
    if (GlobalValues.selectionStart == -1) // a method name is not specified, thus set selection start to the
    ...
    if (etext != null) {
      if (etext.indexOf(".") > -1) {
        // the user has typed a method after the period
