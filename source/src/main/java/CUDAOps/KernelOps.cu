// this file implements CUDA functionality for ScalaLab.
// it is compiled with the NVIDIA nvcc compiler
...
// perform FFT using CUFFT library of CUDA
void cudafft( cufftReal * hinData, int NX, float *hOut ) {
  ...
  /* Copy input float array to the dfftdata device array that consists of cufftComplex data */
...
return 1;
  ...
  // kernels for matrix addition/subtraction/multiply with scalar
  // however, due to tranfer delays, linear time operations like matrix addition / subtraction / multiply with scalar
  // runs usually faster in host CPU 
