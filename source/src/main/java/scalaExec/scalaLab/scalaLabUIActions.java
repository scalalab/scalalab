...
class scalaLabServerAction extends AbstractAction {
    scalaLabServerAction() {
        super("Control IP of ScalaLab server");
    }
...
class scalalabScriptsPathsAction extends AbstractAction {
    scalalabScriptsPathsAction() {
        super(" Browse ScalaLabPath / Remove ScalaLabPath components");
    }
...
class FontAdjusterAction extends AbstractAction {
    public void actionPerformed(ActionEvent e) {
        ...
        int isBold = 0;
        if (choosingFont.isBold()) isBold = 1;
        int isItalic = 0;
        if (choosingFont.isItalic()) isItalic = 1;
...
class configAlphaAction extends AbstractAction {
    configAlphaAction() {
        super("Configuring alpha - transparency parameter");
    }
...
class promptConfigAction extends AbstractAction {
    promptConfigAction() {
        super("Switches Display/Hide working directory at ScalaLab's Console prompt. Current displaying directory  is:  " + GlobalValues.displayDirectory);
    }
...
class browseJavaClassesAction extends AbstractAction {
    browseJavaClassesAction() {
        super("Browse file system for specifyng new ScalaSci ClassPath component");
    }
...
class browseScalaSciFilesAction extends AbstractAction {
    browseScalaSciFilesAction() {
        super("Browse File System for specifying new ScalaSciClassPath component");
    }
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {  // run in  */
                ...
                if (retVal == JFileChooser.APPROVE_OPTION) {    // update the paths according to the selected file
                    ...
                    if (appendPath) {    // path not already exist in ScalaSciClassPath
                    ...
                    if (GlobalValues.ScalaSciUserPaths.contains(SelectedFilePathOnly) == false) {    // path not already exist in ScalaSciUserPaths
    ...
    class commandHistoryAction extends AbstractAction {
        commandHistoryAction() {
            super("Displays the command history");
        }
