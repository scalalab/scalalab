...
//     double increment = xInverted() ? -xTicks : xTicks;
        ...
        double increment;
        if (xInverted()) increment = (xStart - xEnd) / -10;
        else increment = (xEnd - xStart) / 10;
        ...
        if (yInverted()) increment = (yStart - yEnd) / -10;
        else increment = (yEnd - yStart) / 10;
    ...
    protected void zoom(Rectangle rect) {
        ...
        if (rect.height > 0) {
            upperY = rect.y;
            lowerY = rect.y + rect.height;
        } else {
            upperY = rect.y + rect.height;
            lowerY = rect.y;
        }

        double coords[] = new double[4];
        coords[0] = xValue(leftX);
        coords[1] = xValue(rightX);
        coords[2] = yValue(lowerY);
        coords[3] = yValue(upperY);
        zoomForward(coords);
    }
